...
class Medgan(object):
    ...
    def loadData(self, dataPath='', covPath=''):
        data = np.load(dataPath, allow_pickle=True)
        cov = np.load(covPath, allow_pickle=True)

        if self.dataType == 'binary':
            data = np.clip(data, 0, 1)

        #combine data and cov then split into train and valid
        ...
        tDim = self.inputDim + self.covDim
        trainX = train[:,:self.inputDim]
        trainCov = train[:,self.inputDim:tDim]
        validX = valid[:,:self.inputDim]
        validCov = valid[:,self.inputDim:tDim]

        return trainX, validX, trainCov, validCov

    ###apply the autoencoder to learn the salient features
    def buildAutoencoder(self,x_input,x_cov):
        inputs = tf.concat(axis=1, values=[x_input, x_cov])
        decodeVariables = {}
        with tf.variable_scope('autoencoder', regularizer=l2_regularizer(self.l2scale)):
            tempVec = inputs   ###[?,203]
            tempDim = self.inputDim+self.covDim   ###[203]
            i = 0
            for compressDim in self.compressDims:
                W = tf.get_variable('aee_W_'+str(i), shape=[tempDim, compressDim]) #[203,128]
                b = tf.get_variable('aee_b_'+str(i), shape=[compressDim])  #[128]
                tempVec = self.aeActivation(tf.add(tf.matmul(tempVec, W), b))  #[?,128]
                tempDim = compressDim  #128
                i += 1
                
            i = 0
            tempVec = tf.concat(axis=1, values=[tempVec, x_cov])  #[?,131]
            tempDim = tempDim + covDim   #131
            for decompressDim in self.decompressDims[:-1]:
                W = tf.get_variable('aed_W_'+str(i), shape=[tempDim, decompressDim])  #[131,128]
                b = tf.get_variable('aed_b_'+str(i), shape=[decompressDim])  #128
                tempVec = self.aeActivation(tf.add(tf.matmul(tempVec, W), b)) #?,131
                tempDim = decompressDim  # 131
                decodeVariables['aed_W_'+str(i)] = W
                decodeVariables['aed_b_'+str(i)] = b
                i += 1
         
            W = tf.get_variable('aed_W_'+str(i), shape=[tempDim, self.decompressDims[-1]])  #131,200
            b = tf.get_variable('aed_b_'+str(i), shape=[self.decompressDims[-1]])  #200
            decodeVariables['aed_W_'+str(i)] = W
            decodeVariables['aed_b_'+str(i)] = b

            if self.dataType == 'binary':
                x_reconst = tf.nn.sigmoid(tf.add(tf.matmul(tempVec,W),b))  #?,200
    ...
    def buildGenerator(self,x_input, x_cov, bn_train):
        tempVec = tf.concat(axis=1, values=[x_input, x_cov]) 
        tempDim = self.randomDim + self.covDim
        with tf.variable_scope('generator', regularizer=l2_regularizer(self.l2scale)):
            for i, genDim in enumerate(self.generatorDims[:-1]):
                ...
                h3 = self.generatorActivation(h2) #[?,128]
                tempVec = h3 + x_input
                tempDim = genDim
    ...
    def buildGeneratorTest(self, x_input, x_cov, bn_train):
        tempVec = tf.concat(axis=1, values=[x_input, x_cov])
        tempDim = self.randomDim + self.covDim
        with tf.variable_scope('generator', regularizer=l2_regularizer(self.l2scale)):
            for i, genDim in enumerate(self.generatorDims[:-1]):
    ...
    def getDiscriminatorResults(self, x_input, x_cov, keepRate, reuse=False):
        inputs = tf.concat(axis=1, values=[x_input, x_cov])  #[?,203]
        Dim = self.inputDim + self.covDim   #203
        batchSize = tf.shape(inputs)[0]  #?
        inputMean = tf.reshape(tf.tile(tf.reduce_mean(inputs,0), [batchSize]), (batchSize, Dim))  #[?,203]
        tempVec = tf.concat([inputs, inputMean], 1)   #[?,406]
        tempDim = Dim * 2 
        with tf.variable_scope('discriminator', reuse=reuse, regularizer=l2_regularizer(self.l2scale)):
            for i, discDim in enumerate(self.discriminatorDims[:-1]):
                W = tf.get_variable('W_'+str(i), shape=[tempDim, discDim]) #[406,128]
                b = tf.get_variable('b_'+str(i), shape=[discDim]) #[128]
                h = self.discriminatorActivation(tf.add(tf.matmul(tempVec,W),b))  #[?,128]
                h = tf.nn.dropout(h, keepRate)
                tempVec = h
                tempDim = discDim  #128
            W = tf.get_variable('W', shape=[tempDim, 1]) #128,1
            b = tf.get_variable('b', shape=[1])
            y_hat = tf.squeeze(tf.nn.sigmoid(tf.add(tf.matmul(tempVec, W), b)))
        return y_hat
    
    def buildDiscriminator(self,x_real, x_fake, x_cov, keepRate, decodeVariables, bn_train):
        #Discriminate for real samples
        y_hat_real = self.getDiscriminatorResults(x_real, x_cov, keepRate, reuse=False)

        #Decompress, then discriminate for real samples
        tempVec = tf.concat([x_fake, x_cov], 1) #[,131]
        i = 0
        ...
        if self.dataType == 'binary':
            x_decoded = tf.nn.sigmoid(tf.add(tf.matmul(tempVec, decodeVariables['aed_W_'+str(i)]), decodeVariables['aed_b_'+str(i)]))#[200]
        else:
            x_decoded = tf.nn.relu(tf.add(tf.matmul(tempVec, decodeVariables['aed_W_'+str(i)]), decodeVariables['aed_b_'+str(i)]))
        
        
        y_hat_fake = self.getDiscriminatorResults(x_decoded, x_cov, keepRate, reuse=True)
    ...
    def generateData(self,
        ...
        _, x_cov = tf.placeholder('float',[None, self.covDim])
        decodeVariables = self.buildAutoencoder(x_dummy, x_cov)
        x_random = tf.placeholder('float', [None, self.randomDim])
        bn_train = tf.placeholder('bool')
        x_emb = self.buildGeneratorTest(x_random, x_cov, bn_train)
        tempVec = x_emb
        i = 0
        for _ in self.decompressDims[:-1]:
            tempVec = self.aeActivation(tf.add(tf.matmul(tempVec, decodeVariables['aed_W_'+str(i)]), decodeVariables['aed_b_'+str(i)]))
        ...
        with tf.Session() as sess:
            ...
            for i in range(burn_in):
                ...
                output = sess.run(x_reconst, feed_dict={x_random:randomX,x_cov:x_cov, bn_train:True})

            print('generating')
            nBatches = int(np.ceil(float(nSamples)) / float(batchSize))
            for i in range(nBatches):
                randomX = np.random.normal(size=(batchSize, self.randomDim))
                output = sess.run(x_reconst, feed_dict={x_random:randomX,x_cov:x_cov, bn_train:False})
                outputVec.extend(output)

        #outputMat = np.array(outputVec)
        #outDim = outputMat.shape(1)-covDim
        #outDf = pd.DataFrame(outputMat)
        #outDf2 = outDf.iloc[:,0:outDim]
        #outputMat2 = outDf2.values.tolist()
        #np.save(outFile, outputMat2)
    ...
    def train(self,
        ...
        x_cov= tf.placeholder('float',[None,self.covDim])
        x_random= tf.placeholder('float', [None, self.randomDim])
        keep_prob = tf.placeholder('float')
        bn_train = tf.placeholder('bool')

        loss_ae, decodeVariables = self.buildAutoencoder(x_raw, x_cov)
        x_fake = self.buildGenerator(x_random, x_cov, bn_train)
        loss_d, loss_g, y_hat_real, y_hat_fake = self.buildDiscriminator(x_raw, x_fake, x_cov, keep_prob, decodeVariables, bn_train)
        trainX, validX, trainCov, validCov = self.loadData(dataPath, covPath)
            ...
            for epoch in range(nEpochs):
                ...
                for i in range(nBatches):
                    for _ in range(discriminatorTrainPeriod):
                        ...
                        batchCov = trainCov[batchIdx]
                        randomX = np.random.normal(size=(batchSize, self.randomDim))
                        _, discLoss = sess.run([optimize_d, loss_d], feed_dict={x_raw:batchX, x_cov:batchCov,x_random:randomX, keep_prob:1.0, bn_train:False})
                        d_loss_vec.append(discLoss)
                    for _ in range(generatorTrainPeriod):
                        randomX = np.random.normal(size=(batchSize, self.randomDim))
                        _, generatorLoss = sess.run([optimize_g, loss_g], feed_dict={x_raw:batchX, x_cov:batchCov,x_random:randomX, keep_prob:1.0, bn_train:True})
                ...
                for i in range(nBatches):
                    ...
                    batchCov = validCov[batchIdx]
                    randomX = np.random.normal(size=(batchSize, self.randomDim))
                    preds_real, preds_fake, = sess.run([y_hat_real, y_hat_fake], feed_dict={x_raw:batchX,x_cov:batchCov, x_random:randomX, keep_prob:1.0, bn_train:False})
...
def parse_arguments(parser):
    ...
    parser.add_argument('cov_file', type=str, metavar='<cov_file>', help='The path to the covariant matrix.')
...
if __name__ == '__main__':
    ...
    cov = np.load(args.cov_file, allow_pickle=True)
    covDim = len(cov[0])
